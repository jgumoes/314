...
double find_ratio(int fP[], int len_P){
    ...
    */
    ...
    double L[len_P+1];  // if no_i is false, pretend like this has len_P not len_P+1
    // Todo: L doesn't need to exist. get rid of it and add the lengths
    // directly to perimeter
    
    L[0] = fabs((double) pX[0]*8);

    int over_1 = (len_P != 1);
    int dPX[len_P];
    int dPY[len_P];
    if(len_P > 1){
        //Todo: fold these two for loops together and get rid of
        //dPX and dPY
        ...
        for(int i = 0; i < len_P-1; i++){
            int X = dPX[i]; int Y = dPY[i];
            L[i+1] = sqrt(64*(X*X + Y*Y));
        }
    }
    ...
    */
    ...
    if(no_i==1){
        int Y  = pY[len_P-1];
        int X = pX[len_P-1];
        //L[len_P] = sqrt((double) (Y*Y + X*X - 2*Y*X)*32);  
...
double scoot_C(int flat_P[], int num, int len_P){
    ...
    //add_bin(flat_P, num, len_P);
    return(find_ratio(new_fP, len_P));
}

double scoot_np(const void * flat_P, int num, int len_P){
    // just a wrapper for scoot_C that allows it to be called from python with numpy arrays.
    // it's mainly here for testing, and just in case I can't be arsed to do shuffle_C
    // actually, this doesn't function any different from scoot_C
    // which is actually pretty good, because scoot_C is obviously a better name than scoot_np
    int * fP = (int *) flat_P;
    return(scoot_C(fP, num, len_P));
}

void shuffle_C(const void * flat_P, int len_P, int num_start, int num_end){
