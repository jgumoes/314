...
numb = ctypes.c_int(5)
long_num = ctypes.c_double(np.pi)
#cfunc.change_vals.argtypes = [ctypes.c_int, ctypes.c_double]
cfunc.change_vals(ctypes.pointer(numb), ctypes.pointer(long_num))
print("numb = {}\t long_numb = {}".format(numb, long_num))
print("################")

shuffle = ctypes.CDLL("C:\\Users\\jgumo\\Documents\\Scripts\\Python Scripts\\project euler\\314\\C code\\c_shuffle.dll")

shuffle.find_ratio.restype = ctypes.c_double # this corrects the ratio when function doesn't accept arguments and also when it does
shuffle.find_ratio.argtypes = [ctypes.c_void_p, ctypes.c_int] # this gets rid of error when function accepts arguments, fixes everything, might cure aids but haven't gotten that far yet
p_9 = (np.array([123, 135, 146, 158, 168, 178, 188, 198, 207, 249, 247, 244, 240, 235, 230, 223, 215])).ctypes.data
ratio = shuffle.find_ratio(p_9, 9) # *used to* returns ctypes.ArgumentError: argument 1: OverflowError: int too long to convert
...
shuffle.scoot_C_OG.restype = ctypes.c_double
shuffle.scoot_C_OG.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_int]
print(shuffle.scoot_C_OG(p_9, 18, 9))
